...
describe("Protobuf Generators", () => {
    ...
    it("can serialize to base64 string", async () => {
        const rate = {id: "111", bid: 123.45, ask: 248.54, datetime: new Date(2022, 2, 1)};
        const msg = await serializeToBase64(rate);
        expect(msg).to.equal("ChN1bmRlZmluZWQvdW5kZWZpbmVkEc3MzMzM3F5AGeF6FK5HEW9A");
    });

    it("can serialize an array to base64 string", async () => {
        const rate = [{id: "111", bid: 123.45, ask: 248.54, datetime: new Date(2022, 2, 1)}];
        const msg = await serializeArrayToBase64(rate);
    ...
    it("should prepare data before serialization", async () => {
        const data = {
            toTicker: Ticker.Fiat.AUD, 
            fromTicker: Ticker.Fiat.USD, 
            bid: 123.45, ask: 248.54, 
            date: new Date(2022, 1, 1).getTime(), 
            markup: 10
        };
        const preparedData = makeSlice(data);

        const expected: BidAsk = {id: "aud/usd", bid: 123.45, ask: 248.54, markup: 10, datetime: 1643662800000};
